...
$query = "SELECT cu.id, cu.customer_id, cu.registration_no, cu.sr_no, cu.agency_code, cu.branch_id, cu.name,"
        ...
        . "WHERE cu.customer_id = cpm.customer_id AND cu.status = cpm.status AND cu.status=1 AND cpm.status=1 AND cpm.plan_type='FD'";
...
$filename = "ALLCUSTOMERFDLISTDATA_".date('d-M-Y');
