...
$query = "SELECT cu.id, cu.customer_id, cu.registration_no, cu.sr_no, cu.agency_code, cu.branch_id, cu.name,"
        ...
        . "WHERE cu.customer_id = cpm.customer_id AND cu.status = cpm.status AND cu.status=1 AND cpm.status=1 AND cpm.plan_type='MIS'";
...
foreach ($data as $key=>$v) {
    ...
    /*=====Define plan type for excel sheet======*/
	        ...
	        $objPHPExcel->getActiveSheet()->setCellValue(($columnChar++).$columnIncrement, $v['registrationNo']);
                $objPHPExcel->getActiveSheet()->setCellValue(($columnChar++).$columnIncrement, $v['name']);
                $objPHPExcel->getActiveSheet()->setCellValue(($columnChar++).$columnIncrement, $v['father']);
...
$filename = "ALLCUSTOMERRDLISTDATA_".date('d-M-Y');
