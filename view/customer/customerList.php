...
<html>
  <head>
    ...
    <title><?php echo $_SESSION['title'].": All Customer List"; ?></title>
        ...
        <section class="content">
          ...
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title"><i class="fa fa-user-list"></i> List Of All Customer</h3>
              <b> Note : </b>  <span style="color: #e73838;">Customer Delete Button Show will be 5 days ahead of joining date.</span>
            </div><!-- /.box-header -->
            <div class="box-body">
                <?php
                    ...
                    <tbody>
                        <?php
                            ...
                            $query = "SELECT cu.id, cu.customer_id, cu.registration_no, cu.sr_no, cu.agency_code,"
                                    ...
                                    . "FROM customer AS cu, customer_plan_mapping AS cpm, agent_commission_history AS ach WHERE cpm.customer_id = ach.customer_id AND cpm.customer_id = cu.customer_id AND cu.status=1 AND ach.status=1 AND cpm.status=1 group by cu.customer_id ORDER BY cu.customer_id DESC";
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <?php
                                            //$date1 = strtotime($row['date_of_joining']);
                                            //echo $date = strtotime("-7 day", $date1);
                                            if(date('Y-m-d', strtotime('-5 days', strtotime(date("Y-m-d")))) < $row['date_of_joining']){
                                            ?>
                                            <a href="../../controller/customer/customerController.php?action=inactiveCustomer&token=<?php echo $_REQUEST['token']; ?>&cuId=<?php echo $row['customer_id']; ?>&cuName=<?php echo $row['name']?><?php if($row['plan_type']=='MIS'){ echo "&plan_type=MIS";}else{ echo "&plan_type=other";}?>" onclick="return confirm('Are you sure to delete this customer?')"><button class="btn btn-danger btn-xs" data-toggle="tooltip" title="Delete Customer"><i class="fa fa-remove"></i> Delete</button></a>
                                           <?php
                                            }
                                           ?>
                ...
                <a href="exportCustomerList.php?token=<?php echo $_REQUEST['token'];?>" class="btn btn-info btn-xs"> <i class="fa fa-file-archive-o"></i> Export to excel</a>
                <?php
                }else{
                ?>    
