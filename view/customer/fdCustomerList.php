...
<html>
  <head>
    ...
    <title><?php echo $_SESSION['title'].":FD (Fixed Deposit) Customer List"; ?></title>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">FD (Fixed Deposit) Customer List</li>
          </ol>
        </section>
        <!-- Main content -->
        <section class="content">
          ...
          <div class="box box-info">
            <div class="box-header with-border">
              <h3 class="box-title"><i class="fa fa-user-list"></i>FD (Fixed Deposit) Customer List</h3>
            </div><!-- /.box-header -->
            <div class="box-body">
                <?php
                    ...
                    <tbody>
                        <?php
                            ...
                            $query = "SELECT cu.id, cu.customer_id, cu.registration_no, cu.sr_no, cu.agency_code,"
                                    ...
                                    . "FROM customer AS cu, customer_plan_mapping AS cpm, agent_commission_history AS ach WHERE cpm.customer_id = ach.customer_id AND cpm.customer_id = cu.customer_id AND cu.status=1 AND cpm.plan_type='FD' AND ach.status=1 AND cpm.status=1 group by cu.customer_id DESC";
                ...
                <a href="exportFdCustomerList.php?token=<?php echo $_REQUEST['token'];?>" class="btn btn-info btn-xs"> <i class="fa fa-file-archive-o"></i> Export to excel</a>
                <?php
                }else{
                ?>    
                    ...
                    <tbody>
                        <?php
                            ...
                            $query = "SELECT cu.id, cu.customer_id, cu.registration_no, cu.sr_no, cu.agency_code,"
                                    ...
                                    . " cpm.commision_in_per, cpm.interest_rate_in_per, cpm.plan_type, cpm.pay_mode, "
                                    . "cpm.installment_amount, cpm.maturity_return, ach.id, ach.agency_code, ach.customer_id "
                                    . "FROM customer AS cu, customer_plan_mapping AS cpm, agent_commission_history AS ach WHERE cpm.customer_id = ach.customer_id AND cpm.customer_id = cu.customer_id AND cu.status=1 AND ach.status=1 AND cpm.status=1 AND cpm.plan_type='FD' AND cu.branch_id='{$_SESSION['branch_id']}' AND cpm.branch_id='{$_SESSION['branch_id']}' group by cu.customer_id DESC";
                                    $result = $con->query($query);
                            while ($row = mysqli_fetch_array($result)) {
                                ?>
                                    <tr>
